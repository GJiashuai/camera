...
import io.flutter.plugins.camera.features.focusLen.FocusModeLockedWithLensPositionFeature;
...
class Camera
    ...
    void refreshPreviewCaptureSession(
        ...
        Log.i(TAG, "refreshPreviewCaptureSession123");
    ...
    public void setFocusModeLockedWithLensPosition(final Result result, float lensPosition) {
        CameraManager cameraManager = (CameraManager) activity.getSystemService(Context.CAMERA_SERVICE);
        CameraCharacteristics characteristics;
        try {
            characteristics = cameraManager.getCameraCharacteristics(cameraProperties.getCameraName());
        } catch (CameraAccessException e) {
            result.error("getCameraCharacteristicsFailed", e.getMessage(), null);
            return;
        }
        Float minimumFocusDistance = characteristics.get(CameraCharacteristics.LENS_INFO_MINIMUM_FOCUS_DISTANCE);

//        System.out.println("Available focal lengths:");
//        System.out.println("Available focal lengths: " + Arrays.toString(availableFocalLengths));
//        System.out.println("Minimum focus distance: " + minimumFocusDistance);

        final FocusModeLockedWithLensPositionFeature focusModeLockedWithLensPosition = cameraFeatures.getFocusModeLockedWithLensPosition();

        focusModeLockedWithLensPosition.setValue(lensPosition*minimumFocusDistance);
        focusModeLockedWithLensPosition.updateBuilder(previewRequestBuilder);

        refreshPreviewCaptureSession(
                () -> result.success(null),
                (code, message) -> result.error("setFocusModeLockedWithLensPositionFailed", "Could not set lensPosition.", null));
    }
